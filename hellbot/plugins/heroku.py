...
async def restart(event):
    if HEROKU_APP_NAME and HEROKU_API_KEY:
        ...
        await eor(event, f"✅ **Restarted Alexa** \n**Type** `{hl}ping` **after 1 minute to check if I am working !**")
        await event.client.disconnect()


@hell_cmd(pattern="restart$")
async def re(hell):
    event = await eor(hell, "Restarting Alexa ...")
    try:
        await restart(event)
...
async def rel(event):
    await eor(event, "Reloading Alexa... Wait for few seconds...")
    await reload_hellbot()


@hell_cmd(pattern="shutdown$")
async def down(hell):
    event = await eor(hell, "`Turing Off Alexa...`")
    await asyncio.sleep(2)
    await event.edit("**[ ⚠️ ]** \n**Alexa is now turned off. Manually turn it on to start again.**")
...
CmdHelp("power").add_command(
  ...
  "shutdown", None, "Turns off Alexa. Userbot will stop working unless you manually turn it on."
).add_command(
