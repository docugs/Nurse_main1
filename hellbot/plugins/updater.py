...
hellbot_info = "https://raw.githubusercontent.com/Alexa/Plugins/master/BOT-INFO.json"
...
async def hell_info(hellbot_info):
    ...
    _version = infos['BOT-INFO']['version']
    _release = infos['BOT-INFO']['release-date']
    _branch = infos['BOT-INFO']['branch']
    _author = infos['BOT-INFO']['author']
    _auturl = infos['BOT-INFO']['author-url']
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await eor(event, "✅ Successfully updated Alexa!\n\nBot is restarting please wait for a minute.")
...
async def upstream(event):
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your **Alexa** !!")

    if force_update:
        await event.edit(
            "`Force-Updating Alexa. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your Alexa Is UpToDate**\n\n**Version :**  __{hell_ver}__\n**Oɯɳҽɾ :**  {hell_mention}")
    else:
...
async def upstream(event):
    ...
    off_repo = "https://github.com/docugs/hellPlugins"
    os.chdir("/app")
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your Alexa. Basically if you restart dyno it will go back to previous deploy."
).add_command(
  "update build", None, "Hard-Update Your Alexa. This won't take you back to your previous deploy. This will be triggered even if there is no changelog."
).add_info(
  "Alexa Updater."
).add_warning(
  "✅ Harmless Module."
